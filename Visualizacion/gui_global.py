...
def visualizar_secuencias_global(root, entry_seq1, entry_seq2):
    clear_results()
    ...
    if type_seq1 == type_seq2:
        ...
        score, num_alineamientos, alineamientos = NeddlemanWunch.get_results_global(seq1, seq2)
...
def alineamiento_global():
    root = ThemedTk(theme="breeze")  # Tema "arc" de ttkthemes
    root.configure(bg=root.cget('background'))
    root.geometry("690x720")
    root.title("Visualización de Alineamiento Global de Secuencias")

    title_label = ttk.Label(root, text="Alineamiento Global de Secuencias", font=("Arial", 30, "bold"))
    ...
    button_align = ttk.Button(root, text="Alinear", command=lambda: visualizar_secuencias_global(root, entry_seq1, entry_seq2))
